...
export function EACard({ ea, locale, t, index = 0 }) {
  ...
  const facets = ea.spec.marketKey
    ? {
        ...
        'data-access': ea.commercial?.status || 'RESEARCH_ONLY',
      ...
      <a class="eacard__link" href="${href}" aria-label="${ea.name}">
        ...
        <div class="eacard__head">
          ...
          <div class="eacard__badges">
            ...
            ${when(ea.commercial?.plan, () => html`<span class="abadge abadge--${raw(String(ea.commercial.status).toLowerCase())}">${t.commercial.status[ea.commercial.status]}</span>`)}
          </div>
        </div>
